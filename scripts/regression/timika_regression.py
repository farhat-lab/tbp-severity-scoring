...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataframe.iloc[idx]['normalized_log_timika'] # Adjust 'label' to the name of your actual label column
...
wandb.init(project="TBP_XRV_Regression", config={
    ...
    name="Timika-ensemble-regression-performance")
...
for epoch in range(num_epochs):
    ...
    # Compute metrics
    original_labels = [main_df[main_df['patient_id'] == id]['timika'].values[0] for id in val_image_ids]
    ...
    if val_mae < best_val_mae:
        ...
        torch.save(model.state_dict(), 'ensemble-timika-regression.pth')
    else:
        patience_counter += 1
